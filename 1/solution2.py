...
def find_two_numbers_that_sum_to_expected_total(list_of_numbers, expected_total):
    ...
    for index, value in enumerate(list_of_numbers):
        for index2, value2 in enumerate(list_of_numbers):
            for index3, value3 in enumerate(list_of_numbers):
                if index != index2 and index2 != index3:
                    if expected_total == value + value2 + value3:
                        list_of_solution.append((value, value2, value3))
    return list_of_solution

def print_result(list_of_solution):
    if list_of_solution:
        value, value2, value3 = list_of_solution[0]
        print(f'The values that sum to 2020 are {value}, {value2} and {value3}. And the correct answer is {value * value2 * value3}.')
    else:
